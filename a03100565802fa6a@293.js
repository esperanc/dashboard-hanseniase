...
function aText(conf = {}) {
  let { placeholder = "", width = 200, format = (txt)=>txt } = conf;

  const theText = htl.html`<input type=text class=atext placeholder=${format(placeholder)}>`;
...
function aSelect(conf = {}) {
  ...
  if (placeholder && placeholder != "") {
    theSelect.append(
      htl.html`<option value="" disabled selected >${format(placeholder)}`
    );
  }
